...
firebaseHelpers = {
	...
	schoolReached : function() {
		...
		var ls = turf.linestring(userPathLayer.getLatLngs(), newEntry);
		if ( userPathLayer.getLatLngs().length > 1 ) {
			// push to base
			baseRoute.push(ls);
		}
	...
	pathToDB : function() {
      ...
      swal({
	    ...
	    swal({
				title: "Oops...",
				text: "Our data is showing your still inside your privacy buffer. You can keeping route tracking or end your route tracking.",
				type: "warning",
				showCancelButton: true,
				confirmButtonColor: "#0DC083",
				...
				if (isConfirm) {
					swal("Your Still Tracking!", "", "success");
				} else {
					// clear watch
					mapHelpers.clearWatch();
    			schoolHelpers.removeSchool();
